...
{
    $language = new \Zavrsni\Languages\Language;
...
Route::filter('canEditListing', function($route)
{
    $user = Sentry::getUser();

    $id = $route->getParameter('listings');
    $listing = Listing::find($id);

    if (($user->id != $listing->user_id) && !$user->hasAccess('admin'))
    {
        Session::flash('error', trans('users.noaccess'));
        return Redirect::route('home');
    }
